...
class BBoxHead(BaseModule):
    ...
    def loss(self,
        ...
        for i in range(len(labels)):
            # print('************i',i)
            if int(labels[i]) in [1,3,5,7]: label2[i] = 0
            elif int(labels[i]) in [2,4,6,8]: label2[i] = 1
            else: label2[i] = 2 # 背景类
            if int(labels[i]) == 0: label1[i] = 0
            elif int(labels[i]) in [1,2]: label1[i] = 1
            elif int(labels[i]) in [3,4]: label1[i] = 2
            elif int(labels[i]) in [5,6]: label1[i] = 3
            elif int(labels[i]) in [7,8]: label1[i] = 4
            else: label1[i] = 5
        # print('**********************labels1*****************',label1)
        # print('**********************labels2*****************',label2)
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                #     losses.update(loss_cls_)
                # else:
                #     print('*******loss_cls',loss_cls_)
                #     losses['loss_cls'] = loss_cls_
                if self.custom_activation:
                    acc1_ = self.loss_cls.get_accuracy(cls_score, label1)
                    # losses.update(acc1_)
                else:
                    acc1_ = accuracy(cls_score, label1)
        # TODO
        if cls_score1 is not None:
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            if cls_score1.numel() > 0:
                ...
                if isinstance(loss_cls_, dict):
                    ...
                    losses['loss_cls'] = loss_cls_2 + loss_cls_
                if self.custom_activation:
                    acc_ = (acc1_+self.loss_cls.get_accuracy(cls_score1, label2))/2
                    losses.update(acc_)
                else:
                    losses['acc'] = (accuracy(cls_score1, label2)+acc1_)/2
