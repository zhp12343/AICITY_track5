...
def my_non_max_suppression(bboxes, scores, threshold):
    """
    使用非极大值抑制（NMS）去除重叠的边界框

    参数:
    - bboxes: 边界框的坐标，形状为 (N, 4)，N 是边界框的数量，每个边界框由 (x1, y1, x2, y2) 表示。
    - scores: 边界框对应的得分，形状为 (N, )。
    - threshold: IoU（交并比）阈值，用于判断两个边界框是否重叠。

    返回:
    - selected_indices: 选择的边界框的索引列表。
    """
    if len(bboxes) == 0:
        return []
    ...
    while len(sorted_indices) > 0:
        ...
        # 计算当前选定边界框与其他边界框的 IoU
        ious = calculate_iou(bboxes[selected_index], bboxes[sorted_indices[1:]])

        # 移除与选定边界框重叠超过阈值的边界框
        indices_to_remove = [i + 1 for i, iou in enumerate(ious) if iou > threshold]
        sorted_indices = [i for j, i in enumerate(sorted_indices) if j not in indices_to_remove]

    return selected_indices

def calculate_iou(bbox1, bboxes2):
    """
    计算两个边界框的 IoU（交并比）

    参数:
    - bbox1: 第一个边界框的坐标，形状为 (4,)，由 (x1, y1, x2, y2) 表示。
    - bboxes2: 第二个边界框的坐标，形状为 (N, 4)，N 是边界框的数量，每个边界框由 (x1, y1, x2, y2) 表示。
    ...
    x1s, y1s, x2s, y2s = bboxes2[:, 0], bboxes2[:, 1], bboxes2[:, 2], bboxes2[:, 3]
    area_bboxes2 = (x2s - x1s + 1) * (y2s - y1s + 1)
    ...
    ious = area_intersection / np.minimum(area_bbox1 , area_bboxes2)
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if rotated:
            ...
            i = torchvision.ops.nms(boxes, scores, iou_thres) # my_non_max_suppression(boxes, scores, iou_thres) #  # # # NMS TODO
